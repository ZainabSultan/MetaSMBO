...
class EI(AbstractAcquisitionFunction):
    ...
    def _update(self, **kwargs: Any) -> None:
        ...
        self.incumbent = kwargs["incumbent_array"]
        self.X_configurations = kwargs["X"]

    def _compute(self, X: np.ndarray, is_neighbourhood=False, return_configs=False):
        ...
        if not is_neighbourhood:
            X, indices = self.pruner.prune(X, self.X_configurations)
        #self._update()
        ...
        if not self._log:
            ...
            if return_configs:
                return log_ei.reshape((-1, 1)), indices
            else:
                return log_ei.reshape((-1, 1))



    def __call__(self, configurations: list[Configuration], is_neighbourhood=False, return_configs=False):
        ...
        if len(X.shape) == 1:
            X = X[np.newaxis, :]
        if return_configs:
            acq, pruned_X = self._compute(X, is_neighbourhood, return_configs)
        else:
            acq = self._compute(X, is_neighbourhood, return_configs)
