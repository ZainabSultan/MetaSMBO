...
class LocalSearch(AbstractAcquisitionMaximizer):
    ...
    def __init__(
        self,
        configspace: ConfigurationSpace,
        acquisition_function: EI | None = None,
    ...
    def _search(
        ...
        acq_val_candidates_, indices_pruned_candidates = self._acquisition_function(candidates, return_configs=True)
        # convert the pruned configs into Configuration

        # pruned candidates
        candidates = [candidates[idx] for idx in indices_pruned_candidates]

        num_candidates = len(acq_val_candidates_)
        print("num candist", num_candidates)
        if num_candidates == 1:
        ...
        times = []
        print(local_search_steps, "locallsllsl")
        ...
        while np.any(active):
            ...
            if len(neighbors) != 0:
                ...
                acq_val = self._acquisition_function(neighbors, is_neighbourhood=True)
                end_time = time.time()
